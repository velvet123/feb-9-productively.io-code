...
function add_playsinline_script() {
    ?>
...
add_action('wp_footer', 'add_playsinline_script');
...
function get_single_and_related_posts() {
    ...
    if ($related_posts_query->have_posts()) {
        while ($related_posts_query->have_posts()) {
            $related_posts_query->the_post();
            $related_posts .= '<div class="related-post">';
            $related_posts .= '<a href="' . get_the_permalink() .'"><h3>' . get_the_title() . '</h3></a>';
...
function shapeSpace_recent_posts_shortcode($atts, $content = null) {
	...
	extract(shortcode_atts(array(
		'cat'     => '',
		'num'     => '8', // Set the number to 8 to display 8 posts
		'order'   => 'DESC',
		'orderby' => 'post_date',
	), $atts));
	...
	foreach($posts as $post) {
		...
		if ($count % 4 === 0) {
			...
			$output .= '<div class="row recent-blg-cls">'; // Start a new row every 4th post
		}

		$output .= '<div class="col-md-3 recent-post">';
      
		// Check if the post has a featured image
		if (has_post_thumbnail()) {
            $image_attributes = wp_get_attachment_image_src(get_post_thumbnail_id(), 'full');
            $output .= '<div class="post-thumbnail"><img src="' . esc_url($image_attributes[0]) . '" alt="' . esc_attr(get_the_title()) . '"></div>';
        }
        $output .= '<div class="new-cls">';
		$output .= '<h2><a href="'. get_the_permalink() .'">'. get_the_title() .'</a></h2>';
		
		// Limit content to 2 lines (adjust the number of words as needed)
		$output .= '<p>'. wp_trim_words(get_the_excerpt(), 15) .'</p>';
		
		$output .= '<a href="'. get_the_permalink() .'" class="read-more">Read More</a>';
		$output .= '</div>';
		$output .= '</div>';

		if (($count + 1) % 4 === 0 || $count === count($posts) - 1) {
			$output .= '</div>'; // Close the row after the 4th post or on the last post
		}
...
//Newsletter hook
function custom_subscription_confirmation_message($message, $subscription) {
    // Check if the subscription is confirmed
    if ($subscription->get_status() === 'active') {
        // Change the confirmation message
        $message = "Thank you for subscribing! You will now be redirected to our blog.";

        // Add JavaScript redirection script
        $message .= "<script>window.location.href = 'https://productively.io/blog/';</script>";
    }

    return $message;
}

add_filter('woocommerce_email_message_woocommerce_subscription_status-active', 'custom_subscription_confirmation_message', 10, 2);
...
add_action('wp_head', 'custom_single_func');

function custom_single_func() {
    // Get the ID of the post being displayed
    $post_id = get_queried_object_id();

    // Check if it's a single post and if it uses the single.php template
    if (is_single() && is_page_template('single.php')) {
        // Your custom code for the single post with the single.php template goes here
        echo '<meta name="custom-meta" content="your-custom-meta-content">';
    }
}
...
function custom_video_shortcode($atts) {
    ...
    // Build the HTML for the video tag with specified codecs
    $video_html = '<video src="' . esc_url($atts['url']) . '" loop="' . esc_attr($atts['loop']) . '" controls="' . esc_attr($atts['controls']) . '" preload="' . esc_attr($atts['preload']) . '" controlslist="' . esc_attr($atts['controlslist']) . '" playsinline="' . esc_attr($atts['playsinline']) . '" autoplay="' . esc_attr($atts['autoplay']) . '" muted="' . esc_attr($atts['muted']) . '" type="video/' . esc_attr($atts['video_codec']) . '; codecs=' . esc_attr($atts['video_codec'] . ', ' . $atts['audio_codec']) . '"></video>';
