    ...
    if ($category) {
        ...
        if ($posts_query->have_posts()) {
            ...
            while ($posts_query->have_posts()) {
                ...
                $output .= '<a href="' . get_the_permalink() . '" class="read-more"> . get_read_more_text(10) . Read Template</a>';
                $output .= '</div>';
            }
...
// 		// Check if the post has a featured image
// 		if (has_post_thumbnail()) {
// 			$output .= '<div class="post-thumbnail">' . get_the_post_thumbnail(get_the_ID(), 'thumbnail') . '</div>';
// 		}
...
// 	return '<div class="row recent-blg-cls">'. $output .'</div>';
